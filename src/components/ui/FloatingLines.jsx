...
// SỬA LỖI MOBILE: Loại bỏ e.preventDefault() và kiểm tra target
function TouchStart(e) {
  // Nếu người dùng chạm vào input, button, hoặc link -> KHÔNG chặn, để họ bấm được
  if (['INPUT', 'BUTTON', 'A', 'TEXTAREA', 'SELECT'].includes(e.target.tagName)) return;

  if (e.touches.length > 0) {
    // e.preventDefault(); // ĐÃ XÓA DÒNG NÀY ĐỂ KHÔNG CHẶN SỰ KIỆN CLICK/FOCUS
...
function TouchMove(e) {
  if (e.touches.length > 0) {
    // e.preventDefault(); // ĐÃ XÓA DÒNG NÀY
    A.x = e.touches[0].clientX;
    A.y = e.touches[0].clientY;
...
// CẤU HÌNH HIỆU ỨNG (Đã sửa lại màu xanh lá và chất liệu solid)
const X = {
  count: 200,
  colors: [0x166534, 0x22c55e, 0x4ade80], // Màu xanh lá mặc định
  ambientColor: 0xffffff, // Ánh sáng môi trường trắng để màu xanh nổi bật
  ambientIntensity: 0.8,
  lightIntensity: 180,
  materialParams: {
    metalness: 0.4, // Hơi kim loại nhẹ
    roughness: 0.2, // Độ bóng cao
    clearcoat: 1.0, // Lớp phủ bóng loáng
    clearcoatRoughness: 0.1
    // Đã bỏ transmission/transparent để thành bóng đặc (solid)
  },
  minSize: 0.5,
  maxSize: 1.2,
  size0: 1,
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    const o = new g();
    // Material bóng loáng (không còn trong suốt)
...
function createBallpit(e, t = {}) {
  ...
  e.style.touchAction = 'none'; // Vẫn giữ touchAction none để tránh scroll khi đang vuốt bóng (nếu trúng bóng)
